...
import {
  ...
  findAllSuratPerubahanKependudukanSchema,
  UpdateDto,
  updateSchema,
...
export class SuratPerubahanKependudukanController {
  constructor(
    private readonly suratPerubahanKependudukanService: SuratPerubahanKependudukanService,
  ) {}
  ...
  @Get()
  @UsePipes(
    new ZodValidationPipe(findAllSuratPerubahanKependudukanSchema, 'query'),
  )
  ...
  findAll(
    ...
    if (req.user.role !== 'ADMIN') {
      return this.suratPerubahanKependudukanService.findAll(
        query,
        req.user.userId,
      );
    }
  ...
  findOne(@Param('id') id: string, @Request() req: { user: JwtPayload }) {
    if (req.user.role !== 'ADMIN') {
      return this.suratPerubahanKependudukanService.findOne(
        +id,
        req.user.userId,
      );
    }
  ...
  async update(
    ...
    return this.suratPerubahanKependudukanService.update(
      suratId,
      body,
      files,
      userId,
  ...
  removeFile(@Param('id') id: string, @Request() req: { user: JwtPayload }) {
    if (req.user.role !== 'ADMIN') {
      return this.suratPerubahanKependudukanService.removeFile(
        +id,
        req.user.userId,
      );
    }
    return this.suratPerubahanKependudukanService.removeFile(+id);
  }
  ...
  remove(@Param('id') id: string, @Request() req: { user: JwtPayload }) {
    if (req.user.role !== 'ADMIN') {
      return this.suratPerubahanKependudukanService.remove(
        +id,
        req.user.userId,
      );
    }
